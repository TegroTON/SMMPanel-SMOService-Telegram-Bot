# Файл админ панели для ручного создания категории или подкатегории
import os
...
@AddOrRemoveCategoryRouter.message(F.text == 'Добавление и удаление категории и подкатегории')
...
async def BackToMainMenu(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer('Хорошо', reply_markup=Button.ReplyAdminMainKeyboard)
    await callback.message.delete()


# Обрабатываем нажатие на кнопку удаления
@AddOrRemoveCategoryRouter.callback_query(F.data == 'RemoveCategorySubCategory')
async def RemoveCategorySubCategory(callback: CallbackQuery, state: FSMContext):
    # Выводи клавиатуру действий
    await callback.message.answer('Выберите удалить категорию или подкатегорию',
...
async def RemoveCategory(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer('Выберите какую категорию удалить ?',
...
async def RemoveSubCategory(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer('Выберите какую подкатегорию удалить ?',
...
async def Delete_Category(call):
    ...
    await call.message.answer('Хотите добавить или удалить еще ?',
                              reply_markup=Button.AddOrRemoveCategoryKeyboard)
...
async def StateAddCategory(message: Message, state: FSMContext):
    ...
    if message.from_user.id == int(os.getenv('ADMIN_ID')):
        ...
        await message.answer('Хотите ли вы добавить подкатегории для этой категории ?',
...
async def StateAddSubCategory(message: Message, state: FSMContext):
    ...
    if message.from_user.id == int(os.getenv('ADMIN_ID')):
        ...
        await message.answer(res)
        await message.answer('Хотите добавить еще ?', reply_markup=Button.AddSubCategoryKeyboard)
    else:
        await message.answer('Я не знаю такой команды')
